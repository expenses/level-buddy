...
class TextureBuddyPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if context.mode == 'EDIT_MESH' or context.mode == 'OBJECT':
            col = layout.column(align=True)
            col.label(icon="FACESEL", text="Mapping")
                ...
                if f.select:
                    ...
                    if len(obj.data.materials) > matIndex:
                        if obj.data.materials[matIndex] is not None:
                            if len(obj.data.materials[matIndex].texture_paint_images) > 0:
                                image = obj.data.materials[matIndex].texture_paint_images[0]
                                textureWidth = image.generated_width
                                textureHeight = image.generated_height
                            nX = f.normal.x
                            nY = f.normal.y
                            nZ = f.normal.z
                            if nX < 0:
